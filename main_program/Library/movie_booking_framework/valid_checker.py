        ...
        with open(movie_seats_csv_path,'r',newline ='') as ms_csv_r:
            ...
            for lines in ms_csv_r:
                ...
                if DATA_status and not START_status:
                    raise ValueError(
                        f"START or END DIDN'T FOUND!\n"
                        f"File name:{movie_seats_csv}\n"
                        f"File path:{movie_seats_csv_path}\n"
                        f"Line:{line_count}\n"
                        f"This Row: {row[0]}\n"
                        f"LAST Row:{row_head_temp}"
                    )
                if START_status and not CODE_status:
                    raise ValueError(
                        f"MOVIE_CODE DIDN'T FOUND!\n"
                        f"File name:{movie_seats_csv}\n"
                        f"File path:{movie_seats_csv_path}\n"
                        f"Line:{line_count}\n"
                        f"This Row: {row[0]}\n"
                ...
                if CODE_status and START_status and END_status and DATA_status:
                    ...
                    DATA_status = False
                row_head_temp = row[0]
            if START_status and CODE_status: raise ValueError (
                f"END HEADER LOST!\n"
                f"File name:{movie_seats_csv}"
                f"\nFile path:{movie_seats_csv_path}\n"
                f"Line:{line_count}"
            )
            if CODE_status: raise ValueError(
                f"START HEADER AND END HEADER LOST!\n"
                f"File name:{movie_seats_csv}\n"
                f"File path:{movie_seats_csv_path}\n"
                f"Line:{line_count}"
            )
            if DATA_status: raise ValueError(
                f"END HEADER LOST!\n"
                f"File name:{movie_seats_csv}\n"
                f"File path:{movie_seats_csv_path}\n"
                f"Line:{line_count}"
            ...
            if START_status or END_status or CODE_status:
                raise ValueError(
                    f"FORMAT ERROR! COULD NOT PINPOINT THE EXACT ISSUE. "
                    f"PLEASE REVIEW YOUR FILE!\n"
                    f"File name:{movie_seats_csv}\n"
                    f"File path:{movie_seats_csv_path}"
                )
    except ValueError as e:
