...
def seat_dictionary_update (
        seats_dictionary: dict, mt_code_dictionary: dict,
        code_to_update: str, seats_data_to_add: list,
        cinema_code: str = None
) -> dict:
...
def init_all_dictionary():
    global MOVIE_LIST_DICTIONARY,BOOKING_DATA_DICTIONARY,CINEMA_DEVICE_DICTIONARY,MOVIE_SEATS_DICTIONARY,\
        DICTIONARY_INIT_STATUS,CINEMA_SEATS_DICTIONARY,MOVIE_CINEMA_CODE_DICTIONARY,CUSTOMER_DATA_DICTIONARY
    ...
    CUSTOMER_DATA_DICTIONARY = list_dictionary_init(list_csv="customer.csv", code_location=0)
    ...
    MOVIE_CINEMA_CODE_DICTIONARY = primary_foreign_key_dictionary_init(
        list_dict = MOVIE_LIST_DICTIONARY,
        PK_location = 0,
        FK_location = 2
    )
...
def read_list_from_cache (
        dictionary_cache: dict, code: str = "all",header_insert: bool = True,code_location: int=None) -> list:
    cache_list: list = []
    list_for_all: list = []
