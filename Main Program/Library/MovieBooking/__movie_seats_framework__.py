...
def read_movie_seats_list_csv (movie_seats_csv : csv,movie_code : str) -> list:
    list_found = False
    start_status = False
    movie_seat_list = []
    try:
...
def backup_file (original_file : csv,new_file : csv) -> None:
    with open(original_file, 'r', newline='') as ori_file_r, open(new_file, 'w', newline='') as new_file_w:
            oni_file_reader = csv.reader(ori_file_r)
            new_file_writer = csv.writer(new_file_w)
...
if __name__ == '__main__':
    ...
    try:
        movie_seats_list_global = read_movie_seats_list_csv (movie_seats_csv="movie_seat.csv",movie_code="001")
